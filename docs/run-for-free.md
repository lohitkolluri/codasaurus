...
| **4** | A VPS you already pay for | Not new free cloud — just $0 *extra* | Whatever you rented | Still money somewhere, just not *new* money |
...
3. Health check path: `/health`. Deploy until logs say `Database connected (PostgreSQL)` — **not** `(SQLite)` (that means an old image). You should also see `free_tier=true` (Aiven host is auto-detected).
...
| Neon asleep *(if on Neon)* | Open the Neon console once, redeploy |
