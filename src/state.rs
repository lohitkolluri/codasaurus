//! Lightweight state store for tracking review comment IDs.
//! Keyed by `{repo_owner}/{repo_name}/{pr_number}`.
//! Uses SQLite for persistence across bot restarts.
...
impl ReviewState {
    ...
    pub fn get_comment_id(&self, repo: &str, pr_number: i64) -> Result<Option<i64>> {
        ...
        let mut stmt = conn.prepare("SELECT comment_id FROM review_comments WHERE repo_pr = ?1")?;
    ...
    pub fn get_reviewed_sha(&self, repo: &str, pr_number: i64) -> Result<Option<String>> {
        ...
        let key = format!("{}/{}", repo, pr_number);
        let mut stmt = conn.prepare("SELECT head_sha FROM reviewed_commits WHERE repo_pr = ?1")?;
    ...
    /// Store or update the last reviewed commit SHA for a repo+PR combination.
