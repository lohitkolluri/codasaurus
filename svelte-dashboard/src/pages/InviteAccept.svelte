...
<div class="login-page">
  <div class="login-card">
    <div class="login-brand">
      <BrandMark size={40} />
      <h1>Join Codasaurus</h1>
    </div>
    {#if loading}
      <p class="subtitle">Checking invite…</p>
    {:else if !invite}
      <p class="login-error" role="alert">{error || "Invalid invite"}</p>
      <p class="subtitle"><a href="#/login" use:link>Back to sign in</a></p>
    {:else}
      <p class="subtitle">
      ...
      {#if $currentUser}
        <p class="subtitle" style="margin-top:12px">Already signed in as {$currentUser.email}.</p>
      {/if}
    {/if}
  </div>
</div>
